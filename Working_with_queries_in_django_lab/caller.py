...
def find_books_by_genre_and_language(book_genre, language):
    return Book.objects.filter(genre=book_genre, language=language)
...
def find_authors_nationalities():
    authors_with_null_nationality = Author.objects.exclude(nationality__isnull=True)

    result = []

    for author in authors_with_null_nationality:
